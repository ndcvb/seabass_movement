...
source("Scripts/0_Functions_map_base.R")
...
an <- read.csv("Data/animals.csv", stringsAsFactors = F)
...
# Summarise df
df_sum <-  df %>% 
  group_by(station_group, station_name, latitude, longitude) %>% 
  summarise()
...
# Read receiver data
rec <- read.csv("Data/Positions/receivers.csv", stringsAsFactors = F)

# Map
 ggplot() +
  coord_map(xlim = c(2.2,4.2), ylim = c(51,51.9)) +
  theme_bw() +
  theme(panel.background = element_rect(fill = "gray98"),
  ...
  geom_point(data = df_sum, aes(longitude, latitude, fill = station_group), shape = 21, size = 2) +
  scale_fill_manual(values = cols) +
  ...
  geom_label(aes(4,51.4), label = "Scheldt estuary", vjust = 1.8, fill = "#f3943e", size = 4) +
